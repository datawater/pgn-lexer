...
pub fn san_pawn_move(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    match result {
        Some(length) => {
            return Ok((
                &i[length + 1..],
                Token::Move(SS::new_inline_from_ascii(length + 1, &i[0..length + 1])),
            ))
        }
...
pub fn uci_piece_move(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    match result {
        Some(length) => {
            return Ok((
                &i[length..],
                Token::Move(SS::new_inline_from_ascii(length, &i[0..length])),
            ))
        }
...
pub fn san_null_move(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    match result {
        Some(length) => {
            return Ok((
                &i[length + 1..],
                Token::NullMove(SS::new_inline_from_ascii(length + 1, &i[0..length + 1])),
            ))
        }
...
pub fn pgn_escape_comment_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    Ok((
        &i[length..],
        Token::EscapeComment(SS::new_inline_from_ascii(length - 1, &i[1..length])),
    ))
}
...
pub fn pgn_move_annotation_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    Ok((
        &i[length..],
        Token::MoveAnnotation(SS::new_inline_from_ascii(length, &i[0..length])),
    ))
}
...
pub fn pgn_nag_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    match pgn_integer(&i[1..]) {
        Ok((_left, integer)) => Ok((
            &i[integer.len() + 1..],
            Token::NAG(SS::new_inline_from_ascii(
                integer.len(),
                &i[1..integer.len() + 1],
            )),
        )),
...
pub fn pgn_game_result_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    match result {
        Some(length) => {
            return Ok((
                &i[length..],
                Token::Result(SS::new_inline_from_ascii(length, &i[0..length])),
            ))
        }
...
pub fn pgn_start_variation_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    if i[0] == b'(' {
        return Ok((
            &i[1..],
            Token::StartVariation(SS::new_inline_from_ascii(1, &i[0..1])),
        ));
    } else {
...
pub fn pgn_end_variation_token(i: &[u8]) -> IResult<&[u8], Token, PgnError<&str>> {
    ...
    if i[0] == b')' {
        return Ok((
            &i[1..],
            Token::EndVariation(SS::new_inline_from_ascii(1, &i[0..1])),
        ));
    } else {
