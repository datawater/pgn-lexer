...
#[rustfmt::skip]
mod unformated0 {
    pub(crate) fn is_0(i: u8) -> bool {i == b'0'}
    pub(crate) fn is_1(i: u8) -> bool {i == b'1'}
    pub(crate) fn is_2(i: u8) -> bool {i == b'2'}
    pub(crate) fn is_capture(i: u8) -> bool {i == b'x'}
    pub(crate) fn is_dash(i: u8) -> bool {i == b'-'}
    pub(crate) fn is_digit(i: u8) -> bool {i >= b'0' && i <= b'9'}
    pub(crate) fn is_equals(i: u8) -> bool {i == b'='}
    pub(crate) fn is_file(i: u8) -> bool {i >= b'a' && i <= b'h'}
    pub(crate) fn is_letter(i: u8) -> bool {is_lowercase_letter(i) || is_uppercase_letter(i)}
    pub(crate) fn is_lowercase_letter(i: u8) -> bool {i >= b'a' && i <= b'z'}
    pub(crate) fn is_o(i: u8) -> bool {i == b'O'}
    pub(crate) fn is_period(i: u8) -> bool {i == b'.'}
    pub(crate) fn is_piece(i: u8) -> bool {i == b'R' || i == b'N' || i == b'B' || i == b'Q' || i == b'K'}
    pub(crate) fn is_plus_or_hash(i: u8) -> bool {i == b'#' || i == b'+'}
    pub(crate) fn is_rank(i: u8) -> bool {i >= b'1' && i <= b'8'}
    pub(crate) fn is_slash(i: u8) -> bool {i == b'/'}
    pub(crate) fn is_space(i: u8) -> bool {i == b' '}
    pub(crate) fn is_star(i: u8) -> bool {i == b'*'}
    pub(crate) fn is_uppercase_letter(i: u8) -> bool {i >= b'A' && i <= b'Z'}
    pub(crate) fn is_whitespace(i: u8) -> bool {i == b' ' || i == b'\n' || i == b'\r' || i == b'\t'}
    pub(crate) fn is_zero(i: u8) -> bool {i == b'0'}
}
...
macro_rules! match_character {
    ($name:ident, $($matcher:ident),+) => {
        pub(super) fn $name (incoming:&[u8]) -> Option<usize> {
            let mut i: usize = 0;
            $(
                {
...
impl<'a> Iterator for PGNTokenIterator<'a> {
    ...
    fn next(&mut self) -> Option<Token<'a>> {
        self.bytes = remove_whitespace(self.bytes);
        let i = self.bytes;
        let first_byte = i.first()?;

        // Fast path: Determine likely token type based on first byte
        let result = match first_byte {
            b'[' => pgn_tag_symbol_token(i),
            b'\"' => pgn_tag_string_token(i),
            b'{' => pgn_commentary_token(i),
            b'(' => pgn_start_variation_token(i),
            b')' => pgn_end_variation_token(i),
            b'$' => pgn_nag_token(i),
            b'0'..=b'9' => pgn_game_result_token(i).or_else(|_| pgn_move_number(i)).or_else(|_| san_move_token(i)),
            b'%' | b';' => pgn_escape_comment_token(i),
            _ => {
                pgn_game_result_token(i)
                    .or_else(|_| pgn_move_annotation_token(i))
                    .or_else(|_| san_move_token(i)) // Most common, moved earlier
            }
        };
